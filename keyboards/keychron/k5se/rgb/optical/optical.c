...
#include "keymap.h"
    ...
    // iton_bt_battery_level() converts raw values to 0/1/2 index
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (led_state.num_lock)
        rgb_matrix_set_color(NUM_LED_INDEX, RGB_BLUE);
    else
        rgb_matrix_set_color(NUM_LED_INDEX, 0, 0, 0);

    if (led_state.caps_lock)
        rgb_matrix_set_color(CAPS_LED_INDEX, RGB_BLUE);
    else
        rgb_matrix_set_color(CAPS_LED_INDEX, 0, 0, 0);

    if (flags.mac_switch) {
        rgb_matrix_set_color(MAC_LED_INDEX, RGB_BLUE);
        rgb_matrix_set_color(WIN_LED_INDEX, 0, 0, 0);
    } else {
        rgb_matrix_set_color(WIN_LED_INDEX, RGB_BLUE);
